# cmu_112_graphics.py
# Handles TKinter graphics

# version 0.8.6
# Modified by Sanjay Seshan to add scroll detection in mouseScrolled
...
class App(object):
    ...
    def mouseScrolled(app, event): pass  # use event.x, event.y, and event.delta
    ...
    def _redrawAllWrapper(app):
        ...
        try:
            app._canvas.create_rectangle(0,0,1920,1177, fill="white")
    ...
    @_safeMethod
    def _mouseScrolledWrapper(app, event):
        # if (not app._running) or app._paused: return
        # if ((event.x < 0) or (event.x > app.width) or
        #     (event.y < 0) or (event.y > app.height)):
        #     app._mousePressedOutsideWindow = True
        ...
        #     app._mouseIsPressed = True
        #     app._lastMousePosn = (event.x, event.y)
            if (app._methodIsOverridden('mouseScrolled')):
                app.mouseScrolled(event)
                app._redrawAllWrapper()
    ...
    def run(app):
        ...
        if (App._theRoot is None):
            ...
            App._theRoot.bind("<MouseWheel>", lambda event: App._theRoot.app._mouseScrolledWrapper(event))
